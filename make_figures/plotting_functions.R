...
make_the_usual_labels_nice = function(X){
  ...
  try(X$regression_method %<>% gsub("ggrn_docker_backend_sckinetics","sckinetics_dynamics", .))
  try(X$regression_method %<>% gsub("ggrn_docker_backend_dictys","dictys_dynamics", .))
  try(X$regression_method %<>% gsub("ggrn_docker_backend_","", .))
